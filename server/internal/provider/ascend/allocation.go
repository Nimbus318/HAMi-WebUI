...
func Interpret(snapshot *devicecatalog.Snapshot, facts Facts, policy string) Result {
	...
	if mode == ModeUnknown && reason == ReasonNodeModeUnknown && snapshot.Loaded() {
	...
	if cardMemory <= 0 && configured {
		cardMemory = model.MemoryAllocatable
	}
	if cardMemory > 0 && facts.Memory == cardMemory {
...
// Decode returns container slots, init containers first; node may be nil.
func (d Decoder) Decode(pod *corev1.Pod, node *corev1.Node) (map[string][][]Device, error) {
	...
	for _, word := range words {
		...
		for i, value := range strings.Split(pod.Annotations[allocationKey(word)], util.OnePodMultiContainerSplitSymbol) {
			...
			if err != nil {
				return nil, fmt.Errorf("decode %s: %w", allocationKey(word), err)
			}
			for j := range devices {
				facts := &devices[j].Facts
				facts.CardMemory = cardMemory[devices[j].UUID]
				facts.Template = templates.lookup(devices[j].UUID, facts.Memory)
...
// huawei.com/<commonWord> is one list per Pod, so templates are matched by UUID and memory.
...
func (index templateIndex) lookup(uuid string, memory int64) string {
