...
func TestInterpretShapesAndReasons(t *testing.T) {
	...
	tests := []struct {
		...
		{"unread node mode falls back to the catalog default", catalog, Facts{CommonWord: "Ascend910B3", Memory: 16384, Mode: ModeUnknown, ModeReason: ReasonNodeModeUnknown}, "", Result{Shape: ShapeTemplate, Template: "vir05_1c_16g", Cores: 25, Known: true}},
		{"unread node mode without a catalog", nil, Facts{CommonWord: "Ascend910B3", Memory: 16384, Mode: ModeUnknown, ModeReason: ReasonNodeModeUnknown}, "", Result{Shape: ShapeUnknown, Reason: ReasonNodeModeUnknown}},
...
func TestDecodeRejectsATypeThatContradictsTheAnnotation(t *testing.T) {
	pod := ascendPod(map[string]string{"hami.io/Ascend910B3-devices-allocated": "device-0,Ascend910B4,16384,0:"}, 0, 1)
	if _, err := (Decoder{}).Decode(pod, nil); err == nil {
...
func TestDecodeFindsModelsByConfigurationAndRecordsFacts(t *testing.T) {
	...
	pod := ascendPod(map[string]string{
		...
		"hami.io/Ascend910B3-devices-allocated":  ";B3-0,Ascend910B3,16384,0:B3-1,Ascend910B3,32768,0:",
		"huawei.com/Ascend910B3":                 `[{"UUID":"B3-0","temp":"vir05_1c_16g","memory":16384},{"UUID":"B3-1","temp":"vir10_3c_32g","memory":32768}]`,
	...
	first, second := got["Ascend910B3"][1][0].Facts, got["Ascend910B3"][1][1].Facts
	if first.Template != "vir05_1c_16g" || first.CardMemory != 65536 || first.Mode != ModeTemplate || second.Template != "vir10_3c_32g" || second.CardMemory != 0 {
