...
func TestMigDevicesFollowHAMisReservations(t *testing.T) {
	...
	profiles := MigDevices(pod)
	if profiles[0][0].Profile != "1g.10gb" || profiles[0][0].Size != 1 || profiles[1][1].Profile != "3g.40gb" || profiles[1][1].Size != 4 {
	...
	for _, value := range []string{"", "not json", `[{"containerIndex":-1,"profile":"1g.10gb"}]`} {
		got := MigDevices(&corev1.Pod{ObjectMeta: metav1.ObjectMeta{Annotations: map[string]string{MigAllocationsAnnotation: value}}})
		if len(got) != 0 {
