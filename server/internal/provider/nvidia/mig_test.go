...
func TestAllocationsNameAModeOnlyWhenHAMiStatesIt(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{"MIG GPU without its reservation", reserved, registered, 0, "GPU-M", Device{UUID: "GPU-M"}},
		{"reservation for another GPU", reserved, map[string]string{"GPU-X": ModeMig}, 1, "GPU-X", Device{UUID: "GPU-X"}},
		{"reservation on a hami-core GPU", podWith(map[string]string{MigAllocationsAnnotation: `[{"containerIndex":1,"deviceIndex":0,"gpuUUID":"GPU-C","profile":"1g.5gb","placement":{"start":0,"size":1}}]`}), registered, 1, "GPU-C", Device{UUID: "GPU-C"}},
		...
		{"unusable reservations on a MIG GPU", broken, registered, 1, "GPU-M", Device{UUID: "GPU-M"}},
		{"node unread, reservation matches", reserved, nil, 1, "GPU-M", Device{UUID: "GPU-M", Mode: ModeMig, Profile: "3g.40gb", Start: 4, Size: 4}},
		{"node unread, nothing stated", podWith(nil), nil, 0, "GPU-C", Device{UUID: "GPU-C"}},
		...
		{"GPU missing from the registration", podWith(nil), registered, 0, "GPU-Z", Device{UUID: "GPU-Z"}},
	} {
