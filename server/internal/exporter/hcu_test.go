...
func TestHCUContainerTelemetryJoinsRegisteredSerialAndKeepsMissingUsageAbsent(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "whole-card zero-core encoding", present: true, busy: 80, used: 2 * 1024 * 1024 * 1024, annotationCores: 0, wantCores: 100, memoryMiB: 65536, wantMemoryUtil: 3.1},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			const selector = `device_id="serial-01", node="hcu-node", hcu_pod_namespace="research", hcu_pod_name="train", container="worker"`
			computeQuery := fmt.Sprintf("avg(vhcu_utilizationrate{%s}) or avg(hcu_utilizationrate{%s})", selector, selector)
			memoryQuery := fmt.Sprintf("avg(vhcu_usedmemory_bytes{%s}) or avg(hcu_usedmemory_bytes{%s})", selector, selector)
			if tt.present {
