...
| --- | --- | --- |
| Soft split | `huawei.com/vnpu-mode: hami-core`, or a `hami-vnpu-core` node as below | The requested share; 0 reserves none |
| Whole card | Otherwise, memory equals the card's registered memory or the model's `memoryAllocatable` | 100% |
| Template | Otherwise, by the applied template name, or a template of exactly that memory | Template `aiCore` / model `aiCore` |
...
| `node_mode_unknown` | The node's `hami-vnpu-core` annotation and the configuration could not be read. |
...
HAMi installs with `hamiVnpuCore: false`, where this never applies. The mode is
read from the node's current annotation, so an allocation made before the node
changed modes is interpreted with the new one.
