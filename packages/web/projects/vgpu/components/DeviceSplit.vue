      ...
      <div v-if="split.kind === 'mig' && split.slots" class="split-mig">
        ...
        >
          ...
          <t-tooltip v-for="block in split.blocks" :key="block.key" placement="top">
            <template #content>
              <span class="split-tip__title">{{ labelOf(block) }}</span>
              <span class="split-tip__line">{{ detailText(block) }}</span>
...
const detailText = (holder) => joinParts([placementText(holder), memoryText(holder.memoryMiB), computeLabel(holder)]);
...
const describe = (holder) => joinParts([
  labelOf(holder),
  detailText(holder),
...
const summary = computed(() => {
  ...
  const parts = [];
  if (value.kind === 'mig') {
    // Slices are memory slices; a card can have more of them than instances it can hold.
...
const summaryText = computed(() => joinParts(summary.value.map((part) => part.text)));
</script>
...
@media (max-width: 720px) {
  ...
  // Each part wraps onto its own lines, with the holder last.
  .split-rows,
  .split-rows.is-mig,
