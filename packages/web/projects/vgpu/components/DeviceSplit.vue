<template>
  <section class="device-split" :aria-busy="status === 'loading'">
    ...
    <template v-else>
      ...
      <ul v-if="split.holders.length" class="split-rows" :class="{ 'is-shared': split.kind === 'shared' }">
        <li
        ...
        >
          ...
          <span v-if="split.kind !== 'shared'" class="split-row__part">{{ labelOf(holder) }}</span>
...
const labelOf = (holder) => {
  ...
  return t('task.allocation.shape.unknown');
};
const slotText = ({ start, size }) => (size > 1
  ? t('card.split.slices', { start, end: start + size - 1 })
  : t('card.split.slice', { start }));
const sizeText = (holder) => {
  const parts = [memoryText(holder.memoryMiB), `${t('card.split.compute')} ${computeText(holder)}`];
  if (split.value.kind === 'mig' && holder.placement) parts.unshift(slotText(holder.placement));
  return parts.join(' · ');
};
...
const tipTitle = (holder) => [labelOf(holder), sizeText(holder)].join(' · ');
...
const describe = (holder) => [
  `${labelOf(holder)} · ${sizeText(holder)}`,
...
const partClass = (holder) => ({
  ...
  'is-unknown': holder.shape === 'unknown' || holder.shape === '',
});
...
.split-row {
  ...
  &.is-current {
    box-shadow: inset 2px 0 0 var(--split-current);
  }
}
