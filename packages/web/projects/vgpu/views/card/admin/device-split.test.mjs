...
test('one request taking two MIG instances of a card keeps both parts', () => {
  const split = buildDeviceSplit({
    device: a100,
    containers: [{
      podUid: 'pod-c',
      ...
      namespace: 'ml',
      devices: [
        { id: 'GPU-0', template: '3g.20gb', migStart: 0, migSize: 4, allocatedMem: 20480 },
        { id: 'GPU-0', template: '3g.20gb', migStart: 4, migSize: 4, allocatedMem: 20480 },
      ],
    }],
    highlight: { podUid: 'pod-c', container: 'main' },
  });
  assert.deepEqual(split.segments.map(({ kind, size, highlighted }) => [kind, size, highlighted]), [
    ['allocation', 4, true], ['allocation', 4, true],
  ]);
  assert.equal(new Set(split.segments.map(({ key }) => key)).size, 2);
});
